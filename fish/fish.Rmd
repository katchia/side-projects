...
fish_plot <- ggplot(top_fish, aes(x = reorder(species, -values, na.rm = TRUE), y = values, color = species, fill = species, alpha = 0.9)) + 
  ...
  scale_y_log10() +
  coord_flip() +
  labs(title = "Top 3 Most Caught in the Commercial Fishing Industry",
       caption = "Tidy Tuesday | Week 24 | @kat_chia",
       subtitle = "Total fish caught over the past ten years (in thousands)"
      )


final_fish <- fish_plot + theme_ipsum() + scale_color_jama() + scale_fill_jama() + 
  theme(plot.background = element_rect(fill = 'beige'),
        plot.title = element_text(size = 18, family = "Bebas Neue"),
        plot.subtitle = element_text(color = "gray27", family = "Bebas Neue"),
